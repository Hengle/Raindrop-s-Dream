...
namespace Delaunay
{
    ...
    *  Kruskal's spanning tree algorithm with union-find
     ...
     * Note: the sites are implied: they consist of the end Vector2s of the line segments
    */
    public class Kruskal
    {
        ...
        public static List<LineSegment> GetKruskal(List<LineSegment> lineSegments, string type)
        {
            ...
            for (int i = lineSegments.Count; --i > -1; )
            {
                ...
                Node node0 = null;
                if (nodes.ContainsKey(lineSegment.p0.position))
                    node0 = nodes[lineSegment.p0.position];
                Node rootOfSet0;
                if (node0 == null)
                {
                    ...
                    node0.treeSize = 1;
                    nodes.Add(lineSegment.p0.position, node0);
                }
                else
                {
                ...
                Node node1=null;
                if (nodes.ContainsKey(lineSegment.p1.position))
                   node1 = nodes[lineSegment.p1.position];
                Node rootOfSet1;
                if (node1 == null)
                {
                    ...
                    node1.treeSize = 1;
                    nodes.Add(lineSegment.p1.position, node1);
                }
                else
                {
